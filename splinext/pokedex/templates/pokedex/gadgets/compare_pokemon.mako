...
<%def name="title()"><% _ = i18n.Translator(c) %>${_(u"Compare Pokémon")}</%def>
    ...
    <tr class="subheader-row">
        ...
        <th>
            ...
            <a href="${c.create_comparison_link(target=found_pokemon, replace_with=u'')}">
                <img src="${h.static_uri('spline', 'icons/cross-small.png')}" alt="remove" title="${_(u"Remove")}">
            </a>
            <a href="${c.create_comparison_link(target=found_pokemon, move=+1)}">
...
<tbody>
    ...
    <tr class="subheader-row">
        <th colspan="${len(c.found_pokemon) + 1}">${_(u"Breeding + Training")}</th>
    </tr>
    ...
    <tr class="subheader-row">
        <th colspan="${len(c.found_pokemon) + 1}">${_(u"Stats")}</th>
    </tr>
    % for stat in c.stats:
    ...
    <tr class="subheader-row">
        <th colspan="${len(c.found_pokemon) + 1}">${_(u"Flavor")}</th>
    </tr>
